...
namespace YooAsset
{
    internal static class CacheHelper
    {
        ...
        public static EVerifyResult VerifyingCacheFile(VerifyCacheFileElement element, EVerifyLevel verifyLevel)
        {
            try
            {
                ...
                else
                {
                    ...
                    // 解析信息文件获取验证数据
                    CacheFileInfo.ReadInfoFromFile(element.InfoFilePath, out element.DataFileCRC, out element.DataFileHash, out element.DataFileSize);
                }
            }
            catch (Exception)
            {
            ...
            return VerifyingInternal(element.DataFilePath, element.DataFileSize, element.DataFileCRC,element.DataFileHash, verifyLevel);
        }
        ...
        {
            return VerifyingInternal(element.TempDataFilePath, element.FileSize, element.FileCRC,element.FileHash, EVerifyLevel.High);
        }
        ...
        public static EVerifyResult VerifyingRecordFile(CacheManager cache, string cacheGUID)
        {
            ...
            EVerifyResult result = VerifyingInternal(wrapper.DataFilePath, wrapper.DataFileSize, wrapper.DataFileCRC,wrapper.DataFileHash, EVerifyLevel.High);
            return result;
        }

        private static EVerifyResult VerifyingInternal(string filePath, long fileSize, string fileCRC,string fileMD5, EVerifyLevel verifyLevel)
        {
            try
            {
                ...
                else if (verifyLevel == EVerifyLevel.VeryHigh)
                {
